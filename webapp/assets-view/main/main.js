...
var pageid = "1", pageurl = "home.html", pagename = "首页", pageicon = "glyphicon glyphicon-home";
...
$(window).load(function() {
	...
	$.ajax({
		...
		success : function(res) {
			...
			doPageLoad();
			/*
			 * inf = { type : 'QJ', prc : 'aps.com.dao.PageMain.refPersonMsg', }; $.ajax({ type : "POST", url : url, cache : false, data : "inf=" + JSON.stringify(inf), dataType : "json", success :
			 * function(res) { if (res[0].p_e_code != 0) { } else { var html; if (parseInt(res[0].p_m_rows) <= 99) { html = "<a class='dropdown-toggle' data-toggle='dropdown' title='我的消息' href='#'>
			 * <i class='icon fa fa-envelope'></i> <span class='badge'>"; html += +res[0].p_m_rows; html += "</span></a>"; } else { html = "<a class='dropdown-toggle' data-toggle='dropdown'
			 * title='我的消息' href='#'> <i class='icon fa fa-envelope'></i> <span class='badge'>...</span></a>"; } html += " <ul class='pull-right dropdown-menu dropdown-arrow dropdown-messages'>";
			 * for (var index = 1; index < res.length; index++) { if (res[index].mtype == "F") { html += "<li><a href='#'> <img src='res/main/css/images/files.png' class='message-avatar' alt='"; }
			 * else { html += "<li><a href='#'> <img src='res/main/css/images/notes.png' class='message-avatar' alt='"; } html += res[index].mfrom; html += "'> <div class='message'><span
			 * class='message-sender'>"; html += res[index].mfrom; html += "</span> <span class='message-time'>"; if (res[index].mtime < 60) { html += res[index].mtime + " 分钟之前"; } else if
			 * (res[index].mtime < 1440) { html += Math.round(res[index].mtime / 60) + " 小时之前"; } else { html += Math.round(res[index].mtime / 1440) + " 天之前"; } html += "</span> <span
			 * class='message-subject'>"; html += res[index].mtitle; html += "</span> <span class='message-body'>"; html += res[index].mcontent; html += "</span></div></a></li>";
			 * 
			 * if (index >= 5) { break; } } html += " </ul>"; } $("#person-m").html(html);
			 * 
			 * doPageLoad(); }, error : function() { } })
			 */
		},
		error : function() {
		}
